...
import { UnDoneTodoItemUseCase } from './undone-todo-item';

let sut: UnDoneTodoItemUseCase;
...
describe('Done todo item', () => {
  beforeEach(() => {
    ...
    sut = new UnDoneTodoItemUseCase(
      inMemoryTodoItemRepository,
      inMemoryTodoRepository,
    );
  });

  it('should be able to undone todo item', async () => {
    const todo = makeTodo();
    const todoItem = makeTodoItem({ todoId: todo.id, check: true });
    ...
    expect(inMemoryTodoItemRepository.todoItems[0].check).toEqual(false);
  });
  ...
  it('should not be able to undone todo item if todo not exists', async () => {
  ...
  it('should not be able to undone todo item if todo is not yours', async () => {
    const todo = makeTodo();
